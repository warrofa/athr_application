...
import 'package:simpleui/community%20system/modules/screens/certificates/certificates_screen.dart';
import 'package:simpleui/community%20system/modules/screens/profile/profile_screen.dart';
import 'dart:io';
import '../../../certificate/screens/loading.dart';
...
class LayoutCubit extends Cubit<LayoutStates>{
  ...
  List<Widget> layoutScreens = [ const HomeScreen() , const ProfileScreen() , const CertSplashScreen()];
  ...
  void updateUserDataWithoutImage({required String email,required String phoneNumber,required String userName,String? image}){
    ...
    final model = UserModel(image: image?? userData!.image,email: email,phoneNumber: phoneNumber,userName: userName,userID: userData!.userID);
  ...
  void getCommunityImage() async{
    final pickedPostImage = await ImagePicker().pickImage(source: ImageSource.gallery);
  ...
  void createMyCommunity({required String communityName,required String communityDescription}){
    ...
    FirebaseStorage.instance.ref()
      ...
      value.ref.getDownloadURL().then((imageUrl) async {
        final model = CommunityModel(userData!.userName, userData!.userID, userData!.image,communityName,timeNow,imageUrl,communityDescription);
        ...
        await FirebaseFirestore.instance.collection('users')
            .doc(userID ?? userData!.userID).collection('own_communities').add(model.toJson());
  ...
  void addToJoinedCommunity({required CommunityModel communityModel,required String communityID}) async {
    ...
    await FirebaseFirestore.instance.collection('users').doc(userID??userData!.userID).collection('joined_communities')
        .doc(communityID).set(communityModel.toJson()).catchError((error) {emit(FailedToAddToJoinedCommunityState());});
    await getMyAllCommunitiesData();
